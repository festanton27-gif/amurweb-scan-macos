...
const Tile = preload("res://scripts/tile021.gd")
const TILE_SHEET: Texture2D = preload("res://assets/tiles_021.jpg")
...
func _build_textures() -> void:
    ...
    for i in range(6):
        ...
        atlas.region = Rect2((i % 3) * 128, int(i / 3) * 128, 128, 128)
...
func _label(parent: Node, text: String, pos: Vector2, size: Vector2, font_size: int, color := Color.WHITE) -> Label:
    var label := Label.new()
    label.text = text
    label.position = pos
    label.size = size
...
func _show_menu() -> void:
    ...
    logo.position = Vector2(220, 145)
    logo.size = Vector2(280, 280)
    logo.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
    logo.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
    logo.mouse_filter = Control.MOUSE_FILTER_IGNORE
    ui.add_child(logo)
    _label(ui, "ФЕСТИВАЛИМ", Vector2(60, 465), Vector2(600, 58), 38, Color("eafff8"))
    _label(ui, "СОБЕРИ ДВИЖ!", Vector2(60, 520), Vector2(600, 58), 33, Color("8df0d5"))
    _label(ui, "Собери 3 одинаковые фишки в ряд.\nЦель — 12 сердец за 18 ходов.", Vector2(80, 615), Vector2(560, 105), 20, Color("d9eeee"))
    _button(ui, "ИГРАТЬ", Vector2(170, 770), Vector2(380, 76), _start_game)
    _label(ui, VERSION, Vector2(0, 1180), Vector2(720, 30), 13, Color(1, 1, 1, 0.45))
    ...
    for attempt in range(80):
        ...
        for x in range(COLS):
            ...
            for y in range(ROWS):
                ...
                while (x >= 2 and grid[x - 1][y].tile_type == tile_type and grid[x - 2][y].tile_type == tile_type) or (y >= 2 and grid[x][y - 1].tile_type == tile_type and grid[x][y - 2].tile_type == tile_type):
...
func _input(event: InputEvent) -> void:
    ...
    if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
        ...
        if event.pressed:
            ...
            _pointer_release(event.position)
        return
    if event is InputEventMouseMotion and mouse_down and event.device != InputEvent.DEVICE_ID_EMULATION:
        _pointer_drag(event.position)
...
func _swap_tiles(a: Vector2i, b: Vector2i, animate := true) -> void:
    ...
    second.grid_pos = a
    if animate:
        first.move_to(_cell_position(b), 0.10)
        second.move_to(_cell_position(a), 0.10)
...
func _find_matches() -> Array:
    ...
    for y in range(ROWS):
        ...
        for x in range(1, COLS + 1):
            if x == COLS or grid[x][y].tile_type != grid[start][y].tile_type:
                if x - start >= 3:
    ...
    for x in range(COLS):
        ...
        for y in range(1, ROWS + 1):
            if y == ROWS or grid[x][y].tile_type != grid[x][start].tile_type:
                if y - start >= 3:
        ...
        for y in range(ROWS - 1, -1, -1):
            ...
            if tile != null:
                if y != write_y:
                    ...
                    tile.move_to(_cell_position(Vector2i(x, write_y)), 0.11)
                write_y -= 1
