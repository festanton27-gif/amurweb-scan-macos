...
const Tile = preload("res://scripts/tile023.gd")
const TILE_SHEET: Texture2D = preload("res://assets/tiles_023.webp")
const APP_ICON: Texture2D = preload("res://assets/icon_023.png")
...
const TILE_SOURCE_SIZE := 96.0
const VERSION := "DEV 0.2.3"
...
func _build_textures() -> void:
    ...
    for i in range(6):
        ...
        atlas.region = Rect2(
            float(i % 3) * TILE_SOURCE_SIZE,
            float(int(i / 3)) * TILE_SOURCE_SIZE,
            TILE_SOURCE_SIZE,
            TILE_SOURCE_SIZE
        )
...
func _build_game_hud() -> void:
    ...
    shuffle_button = _button(ui, "ПЕРЕМЕШАТЬ", Vector2(265, 930), Vector2(190, 62), _shuffle_board)
    _button(ui, "ЗАНОВО", Vector2(483, 930), Vector2(190, 62), _start_game)
    _label(ui, VERSION, Vector2(0, 1020), Vector2(720, 28), 12, Color(1, 1, 1, 0.35))
...
func _swap_tiles(a: Vector2i, b: Vector2i, animate := true) -> void:
    ...
    if animate:
        first.move_to(_cell_position(b), 0.085)
        second.move_to(_cell_position(a), 0.085)
        ...
        for y in range(ROWS - 1, -1, -1):
            ...
            if tile != null:
                if y != write_y:
                    ...
                    tile.move_to(_cell_position(Vector2i(x, write_y)), 0.095)
                write_y -= 1
...
func _shuffle_board() -> void:
    ...
    if shuffle_button != null:
        shuffle_button.text = "ПЕРЕМЕШАНО"
...
func _rating_for_result(win: bool) -> int:
    if not win:
        return 0
...
func _rating_text(rating: int) -> String:
    if rating >= 3:
        return "★ ★ ★"
    if rating == 2:
        return "★ ★ ☆"
...
func _finish_level(win: bool) -> void:
    ...
    _label(overlay, _rating_text(rating), Vector2(90, 470), Vector2(540, 60), 35, Color("ffd83d"))
